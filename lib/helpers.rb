...
def route_path_relative(item)
  relative_path_to(route_path(item))
end
...
def logo
  if is_blog_page?
    "<a href='#{relative_path_to("/")}'>#{site_name}</a> &laquo;  <a href='#{relative_path_to( "#{blog_url}" )}'>#{blog_name}</a> "
  else
    "<a href='#{relative_path_to("/")}'>#{site_name}</a>"
  end
end

def get_title
  if @item[:title]
   @item[:title] + (is_blog_page? ? ' | ' + blog_name : '')  + ' | ' + site_name 
  else
    ''
  end
end
